...
class image_recognition:
    ...
    def run_detection(self, img, bg, testRun=False):
        ...
        obj_count, detected_points, img_output = self.detectionOutput(img, obj_count, contours_validindex,
                                                                      contours_detected)
        ...
        if (len(validcontours) > 0):
            for i in range(0, len(validcontours)):
                ...
                if self.PRINT_IMG_LABELS == True:
                    ...
                    cv2.putText(img_output, "Point " + str(i), (x - w, y + h), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 0, 0), 1)
                    cv2.putText(img_output, "cx,cy: " + str(self.truncate(cx, 2)) + "," + str(self.truncate(cy, 2)),
                                (x - w, y + h + 9), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
